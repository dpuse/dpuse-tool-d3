import { t as e } from "./linear-D3BD7E1p.js";
import { t } from "./band-DbExrcif.js";
...
function Ct(e, t, n = dt()) {
	...
	let D = (t = !1, n = "update") => {
		...
		i = ne(), a = i, g ? g.renderer !== r.renderer && (g?.destroy(), B(), e.replaceChildren(), g = r.renderer.mount(e, A), h = !1) : g = r.renderer.mount(e, A), g.render(i, {
			...
			tabIndex: r.definition.keyboard === !1 ? -1 : r.tabIndex ?? 0,
			idPrefix: r.idPrefix,
			animation: h ? jt(r.definition.animate, e, n) : void 0
		}), h = !0, b = r.definition.spatialIndex?.(i.points, i);
		let u = c ? Dt(i.points, c) : null;
		if (o = u, u || (l = null), c) {
			s = "restored";
			let e = u ? z(u) : [];
		...
		let t = At(r.definition.focus), n = t ? Mt(t.navigation(i.points), o, e.key) : Nt(i.points, o, e.key);
		...
		if (r.definition.keyboard !== !1 && e.target === g?.element && !o) {
			let e = At(r.definition.focus), t = e ? e.navigation(i.points)[0] : Nt(i.points, null, "Home");
	...
	function ne() {
		...
		return n.render(r.definition, {
			width: e,
			height: r.height ?? (kt(r.aspectRatio) ? e / r.aspectRatio : 320)
		}, { measureText: r.measureText ?? T.measureText });
	}
	function re(e, t, n) {
		let o = ie(), s = At(r.definition.focus);
	...
	function z(e) {
		return At(r.definition.focus)?.group(ie(), e) ?? [e];
	}
	function ae(t, n) {
		let a = Tt(r.definition.tooltip);
		if (!a || !t || !g) {
			v?.hide();
			return;
		}
	...
	function oe() {
		let e = Tt(r.definition.tooltip);
		e ? e.extension === _ ? v?.update(e.options) : B() : (v?.update(wt), v?.hide());
	}
	function B() {
	...
	function se() {
		let e = Tt(r.definition.tooltip)?.options.anchor;
...
function Dt(e, t) {
	...
	return n.find((e) => e.markId === t.markId && Object.is(e.group, t.group) && Ot(e.xValue, t.xValue) && Ot(e.yValue, t.yValue)) ?? n.find((e) => e.markId === t.markId && e.datumIndex === t.datumIndex) ?? n[0] ?? null;
}
function Ot(e, t) {
...
function Mt(e, t, n) {
	let r = t ? e.findIndex((e) => Et(e, t)) : -1, i;
	switch (n) {
		case "ArrowRight":
		case "ArrowDown":
...
function Nt(e, t, n) {
	...
	let i = t ? e.findIndex((e) => Et(e, t)) : -1;
	if (!t || i < 0 || r === 0 || r === 2) return Pt(e, r === 2);
	let a = null, o = -1;
	for (let n = 0; n < e.length; n += 1) {
		let s = e[n];
		if (!s) continue;
		let c = Ft(s, n, t, i);
		r > 0 && c <= 0 || r < 0 && c >= 0 || (!a || r * Ft(s, n, a, o) < 0) && (a = s, o = n);
	}
	return a ?? t;
}
function Pt(e, t) {
...
function Lt(e, t, n) {
	...
	let o = [];
	return Rt(a, i, n ? o : void 0), n ? Ht(e, o, n) : () => {};
}
function Rt(e, t, n) {
	if (zt(e, t, n), !t.firstElementChild) {
		e.textContent !== t.textContent && (e.textContent = t.textContent);
		return;
	}
	let r = [...e.children], i = [...t.children], a = Kt(r), o = qt(i), s = /* @__PURE__ */ new Set(), c = e.firstElementChild;
	i.forEach((t, r) => {
		let i = o[r], l = a.get(i), u;
		l && l.namespaceURI === t.namespaceURI && l.localName === t.localName ? (u = l, s.add(l), u !== c && e.insertBefore(u, c), Rt(u, t, n)) : (u = t.cloneNode(!0), e.insertBefore(u, c), Bt(u, t, n)), c = u.nextElementSibling;
	});
	for (let t of r) !s.has(t) && t.parentElement === e && (n ? Vt(t, n) : t.remove());
}
function zt(e, t, n) {
	...
	for (let i of r) {
		...
		if (r === a) continue;
		let o = n && a !== null && r !== null && It.has(i) ? Wt(a, r) : void 0;
		o && n ? n.push({
			element: e,
			name: i,
			interpolate: o,
...
function Ht(e, t, n) {
	...
	if (!i || !a || o === 0) return Ut(t), () => {};
	let s = 0, c = !1, l, u = Jt(n.easing ?? "ease-out"), d = (e) => {
		if (c) return;
		l ??= e;
...
function Xt(e, t, n) {
	let r = t.idPrefix ?? "", i = t.className ? `ts-chart ${t.className}` : "ts-chart", a = t.ariaDescription ? `<desc>${en(t.ariaDescription)}</desc>` : "", o = n?.renderDefinitions?.(e, r) ?? "", s = e.theme.background === "transparent" ? "" : Zt({
		kind: "rect",
		key: "background",
		x: 0,
		y: 0,
	...
	return `<svg class="${Q(i)}" width="100%" height="100%" viewBox="0 0 ${Z(e.width)} ${Z(e.height)}" role="img" aria-roledescription="chart" aria-label="${Q(t.ariaLabel)}" tabindex="${Z(t.tabIndex ?? 0)}" style="display:block;overflow:visible">${a}${o}${s}${e.nodes.map((e) => Zt(e, n, r)).join("")}</svg>`;
}
function Zt(e, t, n) {
	let r = Qt(e, t, n);
	switch (e.kind) {
		case "group": {
			...
			return `<g${r}${i}${e.focus ? ` data-ts-focus-layer="${e.focus.placement}" visibility="hidden"` : ""}${a?.attributes ?? ""}>${a?.content ?? ""}${e.children.map((e) => Zt(e, t, n)).join("")}</g>`;
		}
		...
		case "label": {
			...
			return `<text${r} x="${Z(e.x)}" y="${Z(e.y)}"${n}${i}${t}${a}${o} font-family="inherit">${en(e.text)}</text>`;
		}
	}
}
function Qt(e, t, n) {
	return `${` data-ts-key="${Q(e.key)}"`}${e.className ? ` class="${Q(e.className)}"` : ""}${e.ariaHidden ? " aria-hidden=\"true\"" : ""}${$t(e.style, t, n)}`;
}
function $t(e, t, n) {
	if (!e) return "";
...
function nn(e, t) {
	...
	let n = e.focus.points.filter((n) => an(n, t, e.focus.match)), r = rn(e.children, n), i = /* @__PURE__ */ new Set();
	return un(r, (e) => i.add(e.key)), i;
}
function rn(e, t) {
	let n = [];
	for (let r of e) {
		if (r.kind !== "group") {
			t.some((e) => cn(r.key, e.key)) && n.push(r);
			continue;
		}
		let e = rn(r.children, t);
		e.length ? n.push({
			...r,
			children: e
...
function an(e, t, n) {
	return n === "x" ? sn(e.xValue, t.primary.xValue) : n === "y" ? sn(e.yValue, t.primary.yValue) : n === "series" ? sn(e.group, t.primary.group) : n === "key" ? e.key === t.primary.key || e.datum === t.primary.datum : n === "group" ? t.group.some((t) => on(e, t)) : on(e, t.primary);
}
function on(e, t) {
	return e === t || e.key === t.key || ln(e.datum) && e.datum === t.datum;
}
function sn(e, t) {
	return O(e) === O(t);
}
...
function un(e, t) {
	for (let n of e) t(n), n.kind === "group" && un(n.children, t);
}
//#endregion
//#region node_modules/@tanstack/charts/dist/mark-state.js
function dn(e, t, n = null) {
	if (!t || !pn(e.nodes)) return { scene: e };
	let r, i = (e, a, o, s, c) => e.map((e) => {
		let l = e.kind === "group" ? e.states : void 0, u = l?.points ?? a, d = l?.definitions ?? o, f = l?.data ?? s, p = l ? xn(l.points) : c, m = u ? yn(e.key, u, p) : bn, h = e.kind !== "group" && d && f && m.length ? mn(e, m, f, d, t, n) : { node: e };
		h.transition && (r = Sn(r, h.transition));
		let g = h.node;
		return g.kind === "group" ? {
			...g,
...
function pn(e) {
	return e.some((e) => e.kind === "group" && (e.states !== void 0 || pn(e.children)));
}
function mn(e, t, n, r, i, a) {
	let o = e, s;
	for (let e of r) {
		let r = hn(t, n, e, i, a);
		r && (o = _n(o, e.style, r), e.transition && (s = Sn(s, e.transition)));
	}
	return {
		node: o,
		transition: s
...
function hn(e, t, n, r, i) {
	if (!(typeof n.when != "function" && n.when.focus === "unmatched" && e.some((e) => an(e, r, "group")))) for (let a of e) {
		let e = {
			datum: a.datum,
			index: a.datumIndex,
			data: t,
		...
		if (typeof n.when == "function" ? n.when(e) : gn(n.when, e)) return e;
	}
}
function gn(e, t) {
	let n = e.source;
	...
	let t = {
		...
		mount(n) {
			...
			return {
				...
				render(t, s) {
					r(), r = Lt(n, e(t, s), s.animation), i = t, a = s, o = void 0;
				},
				clientToScene(e, t, n) {
					return wn(s(), e, t, n);
				},
				paintFocus(t, c) {
					if (!i || !a) return;
					let l = dn(i, t, c), u = o;
					return (l.scene !== i || u) && (r(), r = Lt(n, e(l.scene, a), fn(l.transition ?? u, n))), o = t ? l.transition ?? u : void 0, Tn(s(), l.scene, t), l.scene;
				},
				destroy() {
					r();
				}
...
function Tn(e, t, n) {
	...
	e.querySelectorAll("[data-ts-focus-layer]").forEach((e, t) => {
		let i = r[t], a = i ? nn(i, n) : /* @__PURE__ */ new Set();
...
function En(e) {
	let t = [];
	for (let n of e) n.kind === "group" && (n.focus ? t.push(n) : t.push(...En(n.children)));
	return t;
}
//#endregion
//#region node_modules/@tanstack/charts/dist/dom.js
function Dn(e, t, n = dt()) {
	let r = t.renderSvg ?? tn, i = Cn(r), a = (t) => {
		let n = t.renderSvg ?? tn;
		n !== r && (r = n, i = Cn(r));
...
function On(e, t = {}) {
	...
	return M(({ markIndex: e }) => {
		let r = t.id ?? `bar-y-${e}`, i = P(n, t.x, (e, t) => t), a = Mn(n, t.y ?? t.y2, (e) => typeof e == "number" ? e : void 0), o = P(n, t.z, () => null), s = t.color === void 0 ? o : P(n, t.color, () => null), c = t.z === void 0 && t.color !== void 0 ? s : o, l = t.y1 !== void 0 || t.y2 !== void 0;
		if (l && t.layout?.type === "stack") throw TypeError("A bar with explicit y1 or y2 endpoints cannot also configure a stack layout");
		let u = t.layout?.type === "group", d = t.layout?.type === "stack" ? t.layout : {}, f = !l && !u ? ne(i, a, c, d, "index") : void 0, p = l ? Mn(n, t.y1, () => 0) : f?.starts ?? n.map(() => 0), m = l ? Mn(n, t.y2 ?? t.y, () => void 0) : u ? a : f.ends, h = An(i), g = u || !l && h ? c : o, _ = F(n, t.key, {
			groups: g,
			candidates: [i],
			markId: r,
			warningIdentity: t
		...
		return {
			...
			render: ({ scales: e, chart: o, color: c }) => {
				let l = e.x.bandwidth || jn(e.x, i, o.width, n.length), u = kn(t.layout?.type === "group" ? t.layout : void 0, g, l), d = u?.bandwidth ?? l, f = Math.max(0, t.inset ?? 0), h = [];
...
//#endregion
export { Nn as renderTanStackCharts };

//# sourceMappingURL=tanStackCharts-vvQteV9z.js.map