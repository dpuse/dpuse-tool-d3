...
function bisector(f) {
  ...
  function left2(a2, x, lo = 0, hi = a2.length) {
    if (lo < hi) {
      ...
      do {
        const mid2 = lo + hi >>> 1;
        if (compare2(a2[mid2], x) < 0) lo = mid2 + 1;
        else hi = mid2;
      } while (lo < hi);
    }
    return lo;
...
function axis(orient, scale) {
  var tickArguments = [], tickValues = null, tickFormat2 = null, tickSizeInner = 6, tickSizeOuter = 6, tickPadding = 3, offset2 = typeof window !== "undefined" && window.devicePixelRatio > 1 ? 0 : 0.5, k2 = orient === top || orient === left ? -1 : 1, x = orient === left || orient === right ? "x" : "y", transform2 = orient === top || orient === bottom ? translateX : translateY;
  function axis2(context) {
    ...
    text2.attr(x, k2 * spacing).text(format3);
...
function polygonContains_default(polygon, point2) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      var point1 = ring[j], lambda1 = longitude(point1), phi1 = point1[1] / 2 + quarterPi, sinPhi1 = sin(phi1), cosPhi1 = cos(phi1), delta = lambda1 - lambda0, sign2 = delta >= 0 ? 1 : -1, absDelta = sign2 * delta, antimeridian = absDelta > pi2, k2 = sinPhi0 * sinPhi1;
      sum2.add(atan2(k2 * sign2 * sin(absDelta), cosPhi0 * cosPhi1 + k2 * cos(absDelta)));
  ...
  function clipLine(stream) {
    ...
    return {
      ...
      point: function(lambda, phi) {
        var point1 = [lambda, phi], point2, v = visible(lambda, phi), c4 = smallRadius ? v ? 0 : code(lambda, phi) : v ? code(lambda + (lambda < 0 ? pi2 : -pi2), phi) : 0;
...
function fitExtent(projection3, extent3, object) {
  return fit(projection3, function(b) {
    var w = extent3[1][0] - extent3[0][0], h = extent3[1][1] - extent3[0][1], k2 = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1])), x = +extent3[0][0] + (w - k2 * (b[1][0] + b[0][0])) / 2, y = +extent3[0][1] + (h - k2 * (b[1][1] + b[0][1])) / 2;
...
function conicEqualAreaRaw(y05, y12) {
  ...
  function project2(x, y) {
    var r = sqrt(c4 - 2 * n * sin(y)) / n;
    return [r * sin(x *= n), r0 - r * cos(x)];
  }
  project2.invert = function(x, y) {
    var r0y = r0 - y, l = atan2(x, abs2(r0y)) * sign(r0y);
    if (r0y * n < 0)
...
function azimuthalInvert(angle) {
  return function(x, y) {
    var z = sqrt(x * x + y * y), c4 = angle(z), sc = sin(c4), cc = cos(c4);
    return [
      atan2(x * sc, z * cc),
      asin(z && y * sc / z)
    ];
...
function conicConformalRaw(y05, y12) {
  ...
  project2.invert = function(x, y) {
    var fy = f - y, r = sign(n) * sqrt(x * x + fy * fy), l = atan2(x, abs2(fy)) * sign(fy);
    if (fy * n < 0)
...
function conicEquidistantRaw(y05, y12) {
  ...
  project2.invert = function(x, y) {
    var gy = g - y, l = atan2(x, abs2(gy)) * sign(gy);
    if (gy * n < 0)
...
function maybeZero(x, x12, x2, x3 = identity6) {
...
function inherit2(options = {}, ...rest) {
  let o = options;
  for (const defaults6 of rest) {
    for (const key in defaults6) {
      if (o[key] === void 0) {
        const value = defaults6[key];
...
function createScaleQuantize(key, channels, {
  ...
  if (range3 === void 0) {
    ...
    if (min4 instanceof Date) thresholds = thresholds.map((x) => new Date(x));
  }
...
function facetAnchorTopEmpty(facets, { y: Y3 }, { x, y, empty: empty3 }) {
  if (empty3) return false;
  if (!Y3) return;
  const i = facetIndexOf(Y3, y);
  if (i > 0) return facetEmpty(facets, x, Y3[i - 1]);
}
function facetAnchorBottomEmpty(facets, { y: Y3 }, { x, y, empty: empty3 }) {
  if (empty3) return false;
  if (!Y3) return;
  const i = facetIndexOf(Y3, y);
  if (i < Y3.length - 1) return facetEmpty(facets, x, Y3[i + 1]);
}
function facetAnchorLeftEmpty(facets, { x: X3 }, { x, y, empty: empty3 }) {
  if (empty3) return false;
  if (!X3) return;
  const i = facetIndexOf(X3, x);
  if (i > 0) return facetEmpty(facets, X3[i - 1], y);
}
function facetAnchorRightEmpty(facets, { x: X3 }, { x, y, empty: empty3 }) {
  if (empty3) return false;
  if (!X3) return;
  const i = facetIndexOf(X3, x);
  if (i < X3.length - 1) return facetEmpty(facets, X3[i + 1], y);
}
...
var Mark = class {
  constructor(data, channels = {}, options = {}, defaults6) {
    const {
      facet = "auto",
      facetAnchor,
      fx,
    ...
    if (defaults6 !== void 0) channels = { ...styles(this, options, defaults6), ...channels };
...
function autoHeight({ x, y, fy, fx }, { projection: projection3, aspectRatio }, { width, marginTopDefault, marginRightDefault, marginBottomDefault, marginLeftDefault }) {
...
:where(.${className}-ramp text) {
  ...
  let x;
  const applyRange = round ? (x2, range4) => x2.rangeRound(range4) : (x2, range4) => x2.range(range4);
  ...
  if (interpolate) {
    ...
    x = applyRange(linear2().domain([-1, range3.length - 1]), [marginLeft, width - marginRight]);
...
var RuleX = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x }, offset, 0).call(
      (g) => g.selectAll().data(index2).enter().append("line").call(applyDirectStyles, this).attr("x1", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("x2", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("y1", Y12 && !isCollapsed(y) ? (i) => Y12[i] + insetTop : marginTop + insetTop).attr(
        "y2",
...
var RuleY = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { y: Y3 && y }, 0, offset).call(
...
function ruleX(data, options) {
  let { x = identity6, y, y1: y12, y2, ...rest } = maybeIntervalY(options);
  [y12, y2] = maybeOptionalZero(y, y12, y2);
  return new RuleX(data, { ...rest, x, y1: y12, y2 });
}
function ruleY(data, options) {
  let { y = identity6, x, x1: x12, x2, ...rest } = maybeIntervalX(options);
...
var Text = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    const { rotate } = this;
    const [cx, cy] = applyFrameAnchor(this, dimensions);
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyIndirectTextStyles, this, T, dimensions).call(applyTransform, this, { x: X3 && x, y: Y3 && y }).call(
...
function textX(data, { x = identity6, ...options } = {}) {
  return new Text(data, maybeIntervalMidY({ ...options, x }));
}
function textY(data, { y = identity6, ...options } = {}) {
...
var Vector = class extends Mark {
  constructor(data, options = {}) {
    const { x, y, r = defaultRadius, length: length3, rotate, shape = shapeArrow, anchor = "middle", frameAnchor } = options;
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    const [cx, cy] = applyFrameAnchor(this, dimensions);
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x, y: Y3 && y }).call(
...
function vectorX(data, options = {}) {
  const { x = identity6, ...rest } = options;
  return new Vector(data, { ...rest, x });
}
function vectorY(data, options = {}) {
  const { y = identity6, ...rest } = options;
...
function gridKy(k2, anchor, data, {
  ...
  return axisMark(ruleY, k2, data, { ariaLabel, ariaHidden }, { y, x1: x12, x2, ...gridDefaults(options) });
}
function gridKx(k2, anchor, data, {
  ...
  return axisMark(ruleX, k2, data, { ariaLabel, ariaHidden }, { x, y1: y12, y2, ...gridDefaults(options) });
}
...
function exposeLegends(scales, context, defaults6 = {}) {
  return (key, options) => {
    if (!legendRegistry.has(key)) throw new Error(`unknown legend type: ${key}`);
    if (!(key in scales)) return;
    return legendRegistry.get(key)(scales[key], legendOptions(context, defaults6[key], options), (key2) => scales[key2]);
  };
}
...
function stackY(stackOptions = {}, options = {}) {
  ...
  const [transform2, X3, y12, y2] = stack(x, y, "x", "y", stackOptions, rest);
  return { ...transform2, x1: x12, x: X3, y1: y12, y2, y: mid(y12, y2) };
}
function maybeStackY({ y, y1: y12, y2, ...options } = {}) {
  options = withTip(options, "x");
  if (y12 === void 0 && y2 === void 0) return stackY({ y, ...options });
...
function stack(x, y = one2, kx2, ky2, { offset: offset2, order, reverse: reverse2 }, options) {
  if (y === null) throw new Error(`stack requires ${ky2}`);
  const z = maybeZ(options);
  const [X3, setX] = maybeColumn(x);
  ...
  return [
    basic(options, (data, facets, plotOptions) => {
      ...
      const X4 = x == null ? void 0 : setX(maybeApplyInterval(valueof(data, x), plotOptions?.[kx2]));
      const Y3 = valueof(data, y, Float64Array);
      ...
      for (const facet of facets) {
        ...
        for (const stack2 of stacks) {
          ...
          for (const i of stack2) {
            const y2 = Y3[i];
            if (y2 < 0) yn = Y23[i] = (Y13[i] = yn) + y2;
            else if (y2 > 0) yp = Y23[i] = (Y13[i] = yp) + y2;
            else Y23[i] = Y13[i] = yp;
          }
        }
...
var Tip = class extends Mark {
  ...
  render(index2, scales, values2, dimensions, context) {
    ...
    const g = create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyIndirectTextStyles, this).call(applyTransform, this, { x: X3 && x, y: Y3 && y }).call(
...
function maybeAxis(axes, axis2, axisType, primary, secondary, defaults6, options) {
  if (!axis2) return;
  const both = isBoth(axis2);
  options = axisOptions(both ? primary : axis2, defaults6, options);
  const { line } = options;
  if ((axisType === axisY || axisType === axisX) && line && !isNone(line)) axes.push(frame2(lineOptions(options)));
...
function axisOptions(anchor, defaults6, {
  line = defaults6.line,
  ticks: ticks2,
  tickSize,
  tickSpacing,
...
var AbstractBar = class extends Mark {
  constructor(data, channels, options = {}, defaults6 = barDefaults) {
    super(data, channels, options, defaults6);
    rectInsets(this, options);
  ...
  _width({ x }, { x: X3 }, { marginRight, marginLeft, width }) {
    const { insetLeft, insetRight } = this;
    const bandwidth = X3 && x ? x.bandwidth() : width - marginRight - marginLeft;
    return Math.max(0, bandwidth - insetLeft - insetRight);
  }
  _height({ y }, { y: Y3 }, { marginTop, marginBottom, height }) {
    const { insetTop, insetBottom } = this;
    const bandwidth = Y3 && y ? y.bandwidth() : height - marginTop - marginBottom;
...
var BarY = class extends AbstractBar {
  constructor(data, options = {}, defaults6) {
